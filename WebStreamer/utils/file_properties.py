...
async def gen_link(m: Message,log_msg: Messages, from_channel: bool):
    ...
    dulink_page_link = f"{Var.URL}watch/{get_hash(log_msg)}{log_msg.id}"
    page_link = await get_shortlink(dulink_page_link) 
    print(page_link) 
    dulink_stream_link = f"{Var.URL}{log_msg.id}/{quote_plus(get_name(m))}?hash={get_hash(log_msg)}"
    stream_link = await get_shortlink(dulink_stream_link) 
    print(stream_link)
    Stream_Text=lang.stream_msg_text.format(file_name, file_size, stream_link, Var.AUTH_CHANNEL)
